...
export const getUserChats = () => async (dispatch) => {
    await dispatchAction(dispatch, GET_USERS_CHAT_REQUEST, GET_USERS_CHAT_SUCCESS, GET_USERS_CHAT_ERROR, '/api/chats/user', {
        method: 'GET',
        credentials: 'include',
    });
}
...
export const deleteChat = (chatRoomId) => async (dispatch) => {
    await dispatchAction(dispatch, DELETE_CHAT_REQUEST, DELETE_CHAT_SUCCESS, DELETE_CHAT_ERROR, `/api/chats/delete/${chatRoomId}`, {
        method: 'DELETE',
        credentials: 'include',
    });
};